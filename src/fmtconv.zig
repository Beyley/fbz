...
fn make16Color(comptime RedType: type, comptime BlueType: type, comptime GreenType: type, r: anytype, g: anytype, b: anytype) img.color.Rgb565 {
    ...
    switch (@typeInfo(RedType)) {
        .Int => |int| {
            ...
            if (shift.direction == .Right) {
                destRed = @intCast(r >> shift.amount);
            } else if (shift.direction == .Left) {
                // destRed = @intCast(dest, r << shift.amount);
                destRed = @intCast(shiftLeftShiftingInLSB(dest, r, shift.amount));
            } else {
                destRed = @as(dest, r);
            }
        },
    ...
    switch (@typeInfo(GreenType)) {
        .Int => |int| {
            ...
            //Positive means shift right, negative means shift left
            if (shift.direction == .Right) {
                destGreen = @intCast(g >> shift.amount);
            } else if (shift.direction == .Left) {
                // destGreen = @intCast(dest, g << shift.amount);
                destGreen = @intCast(shiftLeftShiftingInLSB(dest, g, shift.amount));
            } else {
                destGreen = @as(dest, g);
            }
        },
    ...
    switch (@typeInfo(BlueType)) {
        .Int => |int| {
            ...
            //Positive means shift right, negative means shift left
            if (shift.direction == .Right) {
                destBlue = @intCast(b >> shift.amount);
            } else if (shift.direction == .Left) {
                // destBlue = @intCast(dest, b << shift.amount);
                destBlue = @intCast(shiftLeftShiftingInLSB(dest, b, shift.amount));
            } else {
                destBlue = @as(dest, b);
            }
        },
